...
export async function getModerationService(provider) {
  switch(provider){
    ...
    case 'gemini':
      const { GeminiSummaryService } = await import('./services/gemini-service.ts');
      return new GeminiSummaryService();
    default:
      throw new Error(`Unknown moderation provider: ${provider}. Available: openai, perspective, tensorflow, gemini`);
  }
}
